...
from vh_uchen import Vhod_v_uchenika
from sozd_ff import Sozd_new_class

from PyQt5 import QtCore, QtGui, QtWidgets
from PyQt5.QtWidgets import QFileDialog

class zag2(QtWidgets.QMainWindow, no_zaag):
	def __init__(self, parent=None):
		...
		self.Vhod_v_uchenika_window = None
		self.read_mode()

		if self.state == 1:
			self.Vhod_v_uchenika_window = Vhod_v_uchenika(parent=self)
			self.Vhod_v_uchenika_window.show()
			self.hide()
		else:
			self.setup_connections()
			self.show()
	...
	def close_child_window(self):
		...
		if self.Vhod_v_uchenika_window:  # Добавляем закрытие этого окна
			self.Vhod_v_uchenika_window.close()
			self.Vhod_v_uchenika_window = None
